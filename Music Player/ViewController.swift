...
class ViewController: UIViewController, UITableViewDelegate,UITableViewDataSource,AVAudioPlayerDelegate {
    ...
    override func remoteControlReceivedWithEvent(event: UIEvent) {
        if event.type == UIEventType.RemoteControl {
            ...
            case .RemoteControlNextTrack :
                playAudioWithIncrement(1)
            case .RemoteControlPreviousTrack :
                playAudioWithIncrement(-1)
            //earphone can toggle play or pause
            ...
            default :
                NSLog("other controls")
            }
        }
    }
    
    func getLockScreenInfo() {
        if (NSClassFromString("MPNowPlayingInfoCenter") != nil) {
            var songInfo : NSMutableDictionary = NSMutableDictionary()
            songInfo.setObject(readSongNameFromPlist(currentAudioIndex), forKey: MPMediaItemPropertyTitle)
            songInfo.setObject(readFromPlist(currentAudioIndex, keyName: "albumName"), forKey: MPMediaItemPropertyAlbumTitle)
            
            var albumImage : MPMediaItemArtwork = MPMediaItemArtwork(image: UIImage(named : readFromPlist(currentAudioIndex, keyName: "imageName")))
            songInfo.setObject(albumImage, forKey: MPMediaItemPropertyArtwork)
            songInfo.setObject(audioPlayer.currentTime, forKey: MPNowPlayingInfoPropertyElapsedPlaybackTime)
            songInfo.setObject("genius with noname", forKey: MPMediaItemPropertyArtist)
    ...
    func correctProgressValueWhenLocked() {
        if (NSClassFromString("MPNowPlayingInfoCenter") != nil) {
            if let tmp : NSObject =  MPNowPlayingInfoCenter.defaultCenter().nowPlayingInfo {
                var songInfo : NSMutableDictionary = NSMutableDictionary()
                songInfo.setDictionary(tmp as! [NSObject : AnyObject])
                if let currentTime : AnyObject = songInfo.objectForKey(MPNowPlayingInfoPropertyElapsedPlaybackTime) {
                    songInfo.setObject(audioPlayer.currentTime, forKey: MPNowPlayingInfoPropertyElapsedPlaybackTime)
                    MPNowPlayingInfoCenter.defaultCenter().nowPlayingInfo = songInfo as [NSObject : AnyObject]
                }
            }
    ...
    func playAudioWithIncrement(increment:Int) {
        currentAudioIndex += increment
        if currentAudioIndex > audioList.count - 1{
            currentAudioIndex = 0
    ...
    func readSongNameFromPlist(indexNumber: Int) -> String {
        return readFromPlist(indexNumber, keyName: "songName")
    }
    
    func readFromPlist(indexNumber: Int, keyName: String) -> String {
        ...
        var songName = songNameDict.valueForKey(keyName) as! String
        return songName
    }
